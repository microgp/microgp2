...
#define SHORT_OPTIONS "GhHVv:L:R:C:g:i:m:n:l:s:t:T:I:M:A:N:cF:S:p:o:f:+K:P:"

/*
 * INTERNAL PROTOS
 */
static void     MoreHelp(FILE * fout);
...
static float    opCloneScaling = 0.0;
...
static int      opDeterministicFitness = 1;
static int      opMu = 30;
static int      opNu = 0;
static int      opLambda = 20;
static double   opSigma = 0.5;
static double   opTauInitial = 3.0;
static double   opTauDecrement = 0.1;
...
static double   opFitnessLimit = DBL_MAX;
...
static void     ShowHelpAndQuit(int retcode)
{
    ...
    msgMessage(MSG_FORCESHOW | MSG_NOTIME,
	       ...
	       "    --idle-limit %%d  [default: no steady state detection]\n"
	       "    --initial-size %%d  [default: %d]\n"
	       "    --mu %%d  [default: %d]\n"
	       "    --nu %%d  [default: same as mu]\n"
	       "    --lambda %%d  [default: %d]\n"
	       "    --sigma %%f  [default: %0.4f]\n"
	       "    --tau %%f  [default: %0.4f]\n"
	       "    --tau-decrement %%f  [default: %0.4f]\n"
	       "    --self-adaptation-inertia %%f  [default: %0.3f]\n"
	       "    --steady-state-threshold %%f  [default: infinite]",
	       opMaxGenerations, opInitSteps, opMu, opLambda, opSigma, opTauInitial, opTauDecrement,
	       opInertia);
    msgMessage(MSG_FORCESHOW | MSG_NOTIME,
	       "Fitness:\n" "    --concurrent-fitness-calls %%d  [default: %d]\n"
	       "    --fitness-values %%d  [default: %d]\n"
	       "    --max-fitness %%f  [default: unknown]\n"
	       "    --dynamic-fitness  [default: stationary]\n"
	       "    --clone-scaling  [default: %0.2f]\n" 
	       ...
	       opParallelFitnessCalls, opNumFitnesses, opCloneScaling, opFitnessProgram,
	       opFitnessFile, opFitnessOut);
    msgMessage(MSG_FORCESHOW | MSG_NOTIME,
	       "Crash recovery:\n" "    --population-file %%s  [default: %s]\n"
	       "    --load-population %%s  [no default, multiple ok]", opCrashRecoverDump);
    msgMessage(MSG_FORCESHOW | MSG_NOTIME, "");
    exit(retcode);
}
...
int             main(int argc, char *argv[])
{
	...
	/* hidden options */
	{"blues-graph", no_argument, &hiddenGraphStat, 1},
	...
	{"idle-limit", required_argument, 0, 'M'},
	...
	{"tau-decrement", required_argument, 0, 'T'},
	...
	{"clone-scaling", required_argument, 0, 'S'},
	{"fitness-values", required_argument, 0, 'N'},
	{"dynamic-fitness", no_argument, &opDeterministicFitness, 0},
	...
	case 'n':
	    opNu = atoi(optarg);
	    msgMessage(MSG_VERBOSE, "Nu set to %d", opMu);
	    break;
	case 'l':
	...
	case 's':
	    ...
	    break;
	case 't':
	    opTauInitial = atof(optarg);
	    msgMessage(MSG_VERBOSE, "Tau set to %f", opTauInitial);
	    break;
	case 'T':
	    opTauDecrement = atof(optarg);
	    msgMessage(MSG_VERBOSE, "Tau decrement set to %f", opTauDecrement);
	    break;
	case 'I':
    ...
    GEN.CloneScaling = opCloneScaling;
    GEN.IL = IL;
    GEN.Mu = opMu;
    GEN.Nu = max(opNu, opMu);
    GEN.Lambda = opLambda;
    GEN.Inertia = opInertia;
    GEN.IdleLimit = opLambda * opIdleLimit;
    GEN.Sigma = opSigma;
    GEN.TauInitial = opTauInitial;
    GEN.TauDecrement = opTauDecrement;
    ...
    GEN.DeterministicFitness = opDeterministicFitness;
